...
<body class="hold-transition sidebar-mini" style="width: 120%;">
<div class="wrapper">
  <!-- Navbar -->
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="navbar-nav">
      ...
      <a style="    top: -46%; left: 867%;" href='../login/cerrarSesion.php' class="nav-link">Cerrar Sesión</a>
      </li>
    </ul>
  </nav>
  <!-- /.navbar -->
  ...
  <div class="content-wrapper">
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <form method="post" style="text-align:right"> 
              <input style="width: 10%;font-family: monospace;" type="text" placeholder="Nombre" name="xnombre"/>
              <input style="width: 10%;font-family: monospace;" type="text" placeholder="Nombre comercial" name="xnombreComercial"/>
              <select  style="margin-right: 20%; font-family: monospace;" name="xregistrosE">
    ...
    position: absolute;
    top: -10px; margin-right: 20%; font-family: monospace;'>Filtrar</button>
              </form>
            <div style="top: 5%;     text-align: center;" class="card">
              
              <div style="text-align-last: center;" class="card-header">
              
                <h3 style="font-variant: all-petite-caps; font-family: monospace; font-size: 20px;     display: contents;" class="card-title">EMPRESAS</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table style="font-family: monospace;" id="example2" class="table table-bordered table-hover">
                  ...
                  if(isset($_POST['xregistrosE']))
                  {               
                    $limit=$_POST['xregistrosE'];  
                  }
                  if ( isset($_POST['xnombreComercial']))
                  {
                    $nombreC=$_POST['xnombreComercial'];  
                  }                                      
    
                  if(isset($_POST['buscar']))
                  {
                    if(empty($_POST['xnombreComercial']))
                    {
                         $where="WHERE nom_empresa like '".$nombre."%'";
                    }
                    else if (empty($_POST['xnombre']))
                    {
                        $where="WHERE nom_comercial like '".$nombreC."%'";
                    }
                  }                
                  
                  $sql= "SELECT Em.cod_empresa, Em.nom_empresa, Em.nom_comercial, Em.descripcion, 
                  TE.nom_t_empresa, Em.correo, Em.ingresos, Em.camara_comercio
                  FROM EMPRESA Em
                  INNER JOIN TIPO_EMPRESA TE ON Em.cod_t_empresa = TE.cod_t_empresa $where $limit";
                  $resultado=mysqli_query($conexion,$sql);

                  while($mostrar=mysqli_fetch_array($resultado)) 
                  {  
                  ?>
                    <th><?php echo $mostrar['cod_empresa'] ?></th>
                    <th><?php echo $mostrar['nom_empresa'] ?></th>
                    <th><?php echo $mostrar['nom_comercial'] ?></th>
                    <th><?php echo $mostrar['descripcion'] ?></th> 
                    <th><?php echo $mostrar['nom_t_empresa'] ?></th> 
                   
                    
                    <th><?php echo $mostrar['correo'] ?></th>
                    
                    <th><?php echo $mostrar['ingresos'] ?></th>
                    <th>
                    <form  method="post">
                     <button name="buscar" type="submit" class="btn btn-block btn-info" style=" background-color: #0B7984;" >Ver</button>
                   </form>
                    ...
                    <th>
                    <form  method="post">
                     <button name="buscar" type="submit" class="btn btn-block btn-warning" style="background-color: #e8771d;
    color: white;
    border-color: #e8771d;" >Verificar</button>
                   </form>
                         </th>          
                  </tr>
                  </tbody>                  
